<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:58:"/var/www/html/application/admin/view/category/list.tpl.php";i:1599667973;s:57:"/var/www/html/application/admin/view/public/toper.tpl.php";i:1599667973;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
        ...
        <table class="list-table">
          ...
          <?php foreach ($category_list as $v) { ?>
            <tr>
              ...
              <td><?php echo $v['id'] ?></td>
              <td>
                <?php echo $v['sep_name']; if($v['image_url']){ ?>
...
layui.use(['element', 'layer', 'form'], function(){
  ...
  form.on('submit(sort)', function(data){
    ...
    jq.post('<?php echo url("category/sort"); ?>',param,function(data){
      if(data.code == 200){
        layer.close(loading);
  ...
  jq('.list-switch').click(function(){
    var id = jq(this).attr('category-id');
      ...
      var row = jq(this);
      jq.post('<?php echo url("category/menu_switch"); ?>',{'id':id},function(data){
        if(data.code == 200){
  ...
  jq('.update_content_links').click(function(){
    ...
    jq.post('<?php echo url("category/update_content_links"); ?>',{},function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000});
        }else{
